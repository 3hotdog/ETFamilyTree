(function (angular) {
    ...
    app.service("etAnalyze",["$http",function($http){
        this.et_stock_invest=function(et_id,callback) {
            $http.post("/etchart",{
                cache:true,
                params:{et:et_id},
            }).success(function (data) {
               /* var invested=strToJson(data.et.invested);
                var investto=strToJson(data.et.investto);
                var stocks=strToJson(data.et.stock);*/
                let et_name=data.et.et_name;
                this.et=data.et.id;
                let stock=[];//股东
                data.maninvest.forEach(function (man) {
                    stock.push({"name":man.legalp_name})
                });
                data.etinvest.forEach(function (et) {
                    stock.push({"name":et.et_name})
                });
                /*for(var man in stocks){
                    man=stocks[man];
                    stock.push({
                        "name":man.p
                    });
                ...
                data.etinvested.forEach(function (et) {
                    invest.push({"name":et.et_name,"value":et.et_fund})
                });
                /*for(var et in investto){
                    et=investto[et];
                    invest.push({
                        "name":et.et
                    });
